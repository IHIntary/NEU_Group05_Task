...
//    return (SH3001_TRUE);
		...
		if(AppI2c2_MemRead8(devaddr, regaddr, readbuf, length, 10U) == HAL_OK)
		{
			return SH3001_TRUE;
		}
		else
...
//    return (SH3001_TRUE);
		...
		if ((length + 1U) > sizeof(temp))
		{
			return SH3001_FALSE;
		}

		temp[0] = regaddr;
		memcpy(temp+1,writebuf,length);
		if(AppI2c2_MasterTransmit(devaddr, temp, length + 1U, 10U) == HAL_OK)
		{
			return SH3001_TRUE;
		}
		else
