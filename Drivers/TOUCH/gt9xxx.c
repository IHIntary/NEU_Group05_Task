...
static uint8_t g_gt_cmd_wr = GT9XXX_CMD_WR;
static uint8_t g_gt_cmd_rd = GT9XXX_CMD_RD;
...
uint8_t gt9xxx_init(void)
{
    ...
    uint8_t i;
    uint8_t found = 0;
    static const uint8_t probe_table[][3] =
    {
        {0, 0x28, 0x29},
        {1, 0x28, 0x29},
        {0, 0xBA, 0xBB},
    ...
    for (i = 0; i < (sizeof(probe_table) / sizeof(probe_table[0])); i++)
    {
        ...
        g_gt_cmd_wr = probe_table[i][1];
        g_gt_cmd_rd = probe_table[i][2];

        GT9XXX_RST(0);
        GT9XXX_INT_OUT(probe_table[i][0]);
        delay_ms(20);
        GT9XXX_RST(1);
        delay_ms(60);
        ...
        printf("GT9 probe INT=%u WR=0x%02X RD=0x%02X\r\n", probe_table[i][0], g_gt_cmd_wr, g_gt_cmd_rd);
        gt9xxx_rd_reg(GT9XXX_PID_REG, temp, 4);
        temp[4] = 0;
        printf("GT9 PID raw: %02X %02X %02X %02X\r\n", temp[0], temp[1], temp[2], temp[3]);
    ...
    if (found == 0)
    {
        printf("CTP ID invalid: %02X %02X %02X %02X\r\n", temp[0], temp[1], temp[2], temp[3]);
        return 1;
    }

    printf("CTP ID:%s, WR=0x%02X RD=0x%02X\r\n", temp, g_gt_cmd_wr, g_gt_cmd_rd);
...
uint8_t gt9xxx_scan(uint8_t mode, int32_t* x, int32_t* y)
{
    ...
    if ((t % 10) == 0 || t < 10)            /* 空闲时,每进入10次CTP_Scan函数才检测1次,从而节省CPU使用率 */
    {
        ...
        if ((mode & 0XF) && ((mode & 0XF) <= g_gt_tnum))
        {
            ...
            if (tp_dev.x[0] > 1024 || tp_dev.y[0] > 600)  /* 非法数据(坐标超出了) */
            {
