...
void remote_init(void)
{
    ...
    g_tim_handle.Instance = REMOTE_IN_TIMX;                                               /* 定时器1 */
    g_tim_handle.Init.Prescaler = 180 - 1;                                                /* 预分频系数 */
    g_tim_handle.Init.CounterMode = TIM_COUNTERMODE_UP;                                   /* 向上计数 */
    g_tim_handle.Init.Period = 10000;                                                     /* 自动装载值 */
    ...
    HAL_TIM_IC_ConfigChannel(&g_tim_handle, &tim_ic_init_handle, REMOTE_IN_TIMX_CHY);      /* 配置TIM1通道1 */
    HAL_TIM_IC_Start_IT(&g_tim_handle, REMOTE_IN_TIMX_CHY);                                /* 开始捕获TIM的通道值 */
...
{
    HAL_TIM_IRQHandler(&g_tim_handle);      /* 定时器共用处理函数 */
}
...
void HAL_TIM_IC_CaptureCallback(TIM_HandleTypeDef *htim)
{
    if (htim->Instance == REMOTE_IN_TIMX)
    {
        ...
        if (RDATA)                                                                                                 /* 上升沿捕获 */
        {
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_FALLING);    /* CC1P=1 设置为下降沿捕获 */
            __HAL_TIM_SET_COUNTER(&g_tim_handle, 0);                                                               /* 清空定时器值 */
            g_remote_sta |= 0x10;                                                                                  /* 标记上升沿已经被捕获 */
        }
        else                                                                                                       /* 下降沿捕获 */
        {
            dval = HAL_TIM_ReadCapturedValue(&g_tim_handle, REMOTE_IN_TIMX_CHY);                                   /* 读取CCR1也可以清CC1IF标志位 */
            __HAL_TIM_SET_CAPTUREPOLARITY(&g_tim_handle, REMOTE_IN_TIMX_CHY, TIM_INPUTCHANNELPOLARITY_RISING);     /* 配置TIM1通道1上升沿捕获 */
